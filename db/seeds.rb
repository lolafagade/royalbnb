...
moana = User.create!(name: 'Moana', email: 'moana@mauisunshine.com', password: "moana!", avatar: File.open(Rails.root.join('db/fixtures/princesses/moana_avatar.jpg')))
alice = User.create!(name: 'Alice', email: 'alice@wonderland.com', password: "alice!", avatar: File.open(Rails.root.join('db/fixtures/princesses/alice_avatar.jpeg')))
pocahontas = User.create!(name: 'Pocahontas', email: 'poca@cherokeelove.com', password: "pocahontas!", avatar: File.open(Rails.root.join('db/fixtures/princesses/pocahontas_avatar.jpg')))
...
prince: 'Alladin', location: 'India', equipment: 'genie in a lamp, tiger', vehicle: 'flying carpet', price_per_night: '5',
...
prince: 'Prince Eric', location: 'Bermuda, USA', equipment: 'palms, seashell bikinis', vehicle: 'No need for a vehicle, swim!',
price_per_night: '12', user_id: ariel.id, photo: File.open(Rails.root.join('db/fixtures/castles/ariel-castle.jpg')))

snowwhite_castle = Castle.create!(name: "Forest Cottage", description: "A lovely cottage in the forest, where you'll sing happily with all your animal friends.",
location: 'Kartäuserstraße 99, 79104 Freiburg im Breisgau, Germany', equipment: 'Seven dwarfs', vehicle: 'wheelbarrow', prince: 'nope', price_per_night: '3', user_id: snowwhite.id, photo: File.open(Rails.root.join('db/fixtures/castles/snowwhite-cottage.jpg')))
...
Did you always dreamed about the idea to sleep under the starry sky? Then this is perfect!.", prince: 'none', location: 'Hawaii, USA', equipment: 'An angry grandmother, tattoos, weird dancing ritual with sticking your tongue out', vehicle: 'Kanoe', price_per_night: '3',
user_id: moana.id, photo: File.open(Rails.root.join('db/fixtures/castles/moana_castle.jpg')))
...
user_id: alice.id, photo: File.open(Rails.root.join('db/fixtures/castles/alice_castle.jpg')))
...
# user_id: pocahontas.id, photo: File.open(Rails.root.join('db/fixtures/castles/pocahontas_castle.png')))
